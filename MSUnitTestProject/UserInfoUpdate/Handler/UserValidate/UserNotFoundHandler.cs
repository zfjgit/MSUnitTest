...
namespace MSUnitTestProject.UserInfoUpdate.Handler.UserValidate
{
    ...
    {
        public UserNotFoundHandler(short colorIndex, Dictionary<short, int> counts, AbstractUserRowHandler<IList<User>> nextHandler) : base(colorIndex, counts, nextHandler)
        {
        }
        ...
        public override bool Handle(HandleContext<IList<User>> context)
        {
            if (context.Data == null || context.Data.Count <= 0)
            {
                ...
                Console.WriteLine("User not found: " + context.Name);
                return false;
            }
